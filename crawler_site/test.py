...
设置文件储存的路径 
'''
path = ".\\total_content_comments.csv"  # 总的微博博文与评论的全部信息
...
headers = {
    ...
    'Referer': 'https://s.weibo.com/weibo?q=%E5%BC%A0%E6%98%8E%E6%9D%B0&typeall=1&suball=1×cope=custom:2021-10-25:2021-11-18-17&Refer=g',
...
def get_title_id():
    for page in range(1, 50):  # 每个页面大约有9个话题
        ...
        time.sleep(1)
        # 该链接通过抓包获得
        api_url = "https://s.weibo.com/weibo?q={}&typeall=1&suball=1×cope=custom:{}-{:0>2d}-{:0>2d}-{}:{}-{:0>2d}-{:0>2d}-{}&Refer=g&page={}".format(
...
end_date = pd.to_datetime('2021/11/26')
